...
if(isset($login) && isset($stickers) && isset($coins) && isset($emeralds) && isset($card_need) && isset($level) && isset($card))
		{
	...
	foreach(mysqli_fetch_row($d) as $dd)
{
  $my_emeralds = $dd;
	...
	$a = mysqli_query($link, "SELECT Stickers FROM DataPets WHERE login='".$login."'");
	foreach(mysqli_fetch_row($a) as $aa)
{
  $answer = $aa;
}			
$pos = strpos($answer, $stickers."_1");
if($pos === false){
	$answer2 = $answer.$stickers."_1"."/".$stickers."_2"."/".$stickers."_3"."/".$stickers."_4"."/";
	mysqli_query($link, "UPDATE DataPets SET Stickers='".$answer2."' WHERE login='".$login."'");
}
